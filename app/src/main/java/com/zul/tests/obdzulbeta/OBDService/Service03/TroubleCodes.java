package com.zul.tests.obdzulbeta.OBDService.Service03;
...
public class TroubleCodes extends OBDCommand {


    public TroubleCodes(BluetoothManager btManage, android.os.Handler handler) {
        super("03\r\n", btManage,  handler);
        btManage.setHandler(obdCommandHandler);
    }

    public TroubleCodes(TCPSocketManager tcpSocketManager, android.os.Handler handler) {
    ...
    public String formatAnwser(ArrayList<String> message) {
        if ( message.get(0).contains("43")) {
            String dtcs = message.get(0).substring(message.get(0).indexOf("43"));
            String listOfDTC = "";
            dtcs = dtcs.replace(" ", "");
